...
prettyPrintProgramPrime (g : t, s) = prettyPrintProgramPrime (t, s ++ prettyPrintGlobal g ++ "\n")
...
prettyPrintArgsPrime ((Arg(ty, i)) : t, s) = prettyPrintArgsPrime (t, s ++ "(" ++ prettyPrintType ty ++ ", \"" ++ i ++ "\")")
...
prettyPrintStmt (StmtVarDef (ty, i, e)) = 
    ...
    prettyPrintExpr e ++ 
    ")"
prettyPrintStmt (StmtVarAss (i, e)) = "SVarAssign (\"" ++ i ++ "\", " ++ prettyPrintExpr e ++ ")"
...
prettyPrintExpr (ExprIdent i) = "EVar (\"" ++ i ++ "\")"
prettyPrintExpr (ExprInt n) = "EInt (" ++ (show n) ++ ")"
prettyPrintExpr (ExprChar '"')  = "EChar ('\\\"')"
prettyPrintExpr (ExprChar c) = "EChar (" ++ (show c) ++ ")"
prettyPrintExpr (ExprString s) = "EString (\"" ++ s ++ "\")"
prettyPrintExpr (ExprBinOp (op, e1, e2)) = 
    ...
    ", " ++ prettyPrintExpr e2 ++
    ")"
prettyPrintExpr (ExprCall (i, e)) = prettyPrintExprCall (e, ("ECall (\"" ++ i ++ "\", {"))
